...
indir_calib_analysis<-paste0(here(), '/test/calibration_results/', 
                             region_name_temp)
outdir_graphs <-paste0(indir_calib_analysis, '/distribution_of_accepted_points_graphs')
indir_filtered_calib_sets<-paste0(here(), '/test/calibration_results')
...
#read in remaining stats
  ...
  remaining_calib_sets_df<-read.csv(paste0('general_accepted_calibration_sets_ref_df', 
                                           itr, 
                                           '.csv'))
  
  best_results_df_plot_dist<-best_results_df_plot_dist%>%
    filter(general_sim_id %in% remaining_calib_sets_df$sim_id)
}
