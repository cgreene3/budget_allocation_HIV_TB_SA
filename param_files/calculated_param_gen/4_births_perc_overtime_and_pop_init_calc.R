...
#data frames
birth_perc_df_all<-data.frame()

lapply(unique(pop_estimates_df$region_name), function(i){
  
  pop_estimates_df_temp<-pop_estimates_df%>%
    filter(region_name == i)%>%
    select(-c('region_name'))
  
  HIV_prev_temp<-HIV_prev%>%
    filter(location == i)%>%
    select(-c('location'))
  
  #aging into unifected (TB 1), latent recent (TB 2 and TB 3), active (TB 6)
  ...
  uninfected_df_no_hiv<-all_prev_num_estimates_df%>%
    ...
    ungroup()%>%
    left_join(pop_estimates_df_temp, by = c('year', 'sex'))%>%
    mutate(n_uninfected = expected_total_pop - total_disease_pop,
           TB_compartment = 1,
           DR_compartment = 1)%>%
    left_join(HIV_prev_temp, by = c('year', 'sex'))%>%
  ...
  for (year in unique(pop_estimates_df_temp$year)){
    for (t in 1:7){
      for (r in 1:2){
        for (h in 1:4){
          for (g in 1:2){
  ...
  birth_perc_df3$region_name<-i
  birth_perc_df3<-birth_perc_df3%>%
    filter(year <= 2017)
  
  
  if(nrow(birth_perc_df_all) == 0){
    birth_perc_df_all<<-birth_perc_df3
  } else{
    birth_perc_df_all<<-rbind(birth_perc_df_all, birth_perc_df3)
  }
  
})
setwd(outdir)
write.csv(birth_perc_df_all, 'birth_perc_df_overtime.csv')

#check everything adds up to 1
test<-birth_perc_df_all%>%group_by(year, region_name)%>%summarise(sum(prop_of_pop))

####pop init df###
pop_init_df_1940<-birth_perc_df_all%>%
  filter(year == 1989)%>%
  ...
  select(c('region_name', 'TB_compartment', 'DR_compartment', 
           'HIV_compartment', 'G_compartment',
           'compartment_id', 'dcompartment_id', 'total_pop'))

test<-pop_init_df_1940%>%group_by(region_name)%>%summarise(sum(total_pop))
