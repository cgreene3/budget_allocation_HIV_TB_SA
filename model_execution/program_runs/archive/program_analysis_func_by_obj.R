...
#27 solution sets per run
args_temp<-as.integer(args)
args_start<-1+(args_temp-1)*27
args_end<-args_start+26
...
lapply(unique(all_solution_sets_df$solution_id[args_start:args_end]),
  ...
  lapply(files, function(f){
    ...
    solution_vals<-sim_set_df_melt%>%
      ...
      mutate(mean_tb_inc = mean(TB_inc_per_Y_100k_ppl),
             mean_hiv_prev = mean(HIV_prev_Y_100k_ppl),
             mean_tbhiv_mort = mean(TBHIV_mort_per_Y_100k_ppl))%>%
      mutate(diff_TB_inc = abs(TB_inc_per_Y_100k_ppl-mean_tb_inc),
             diff_HIV_prev = abs(HIV_prev_Y_100k_ppl-mean_hiv_prev),
             diff_TBHIV_mort = abs(TBHIV_mort_per_Y_100k_ppl-mean_tbhiv_mort))%>%
      summarise(equity_TB_inc_Y = max(diff_TB_inc),
                equity_HIV_prev_Y =max(diff_HIV_prev),
                equity_TBHIV_mort_Y = max(diff_TBHIV_mort),
                total_TBHIV_mort_num = sum(TBHIV_mort_num_total),
                total_cost = sum(program_cost))
  ...
  func_mean_sd_df<-melt(solution_vals_all, id = c("sim_id"))
  func_mean_sd_summ<-func_mean_sd_df%>%
